...
std::vector<double> MCTS::get_action_probs(Go *go_or_chess, double temp) {
  // submit simulate tasks to thread_pool
  std::vector<std::future<void>> futures;

  for (unsigned int i = 0; i < this->num_mcts_sims; i++) {
    auto game = std::make_shared<Go>(*go_or_chess);
    auto future =
  ...
  // calculate probs
  std::vector<double> action_probs(this->action_size, 0);
...
int MCTS::get_best_action_from_prob(std::vector<double> action_probs) {
    ...
    for (unsigned int i = 0; i < this->action_size; i++) {
...
int MCTS::get_best_action(Go *go_or_chess) {
  // submit simulate tasks to thread_pool
  std::vector<std::future<void>> futures;
...
void MCTS::simulate(std::shared_ptr<Go> game) {
