...
#endif
  size_t input_tensor_size = Go::input_size;
...
std::future<NeuralNetwork::return_type> NeuralNetwork::commit(Go *game)
{
...
void NeuralNetwork::infer()
{
  ...
  size_t input_tensor_size = input_node_dims[0] * Go::input_size;
  ...
  for (unsigned int i = 0; i < promises.size(); i++)
  {
    ...
    std::vector<double> prob(P + i * Go::action_size, P + (i+1)*Go::action_size);
    std::vector<double> value{V[i]};
    //assert(prob.size() == BORAD_SIZE * BORAD_SIZE); //for gomoku

    for (int j = 0; j < Go::action_size; j++)
    {
