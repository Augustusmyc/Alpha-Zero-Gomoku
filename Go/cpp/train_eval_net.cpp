...
{
    MCTS ma(a, NUM_MCT_THREADS, C_PUCT, a_mct_sims, C_VIRTUAL_LOSS, Go::action_size);
    MCTS mb(b, NUM_MCT_THREADS, C_PUCT, b_mct_sims, C_VIRTUAL_LOSS, Go::action_size);
    int step = 0;
    ...
    if ((game_state.second == Go::FirstColor && a_first) || (game_state.second == Go::SecondColor && !a_first))
    {
        cout << "winner = a" << endl;
        win_table[0]++;
    }
    else if ((game_state.second == Go::FirstColor && !a_first) || (game_state.second == Go::SecondColor && a_first))
    {
        cout << "winner = b" << endl;
        win_table[1]++;
    }
