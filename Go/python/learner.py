...
class Learner():
    def __init__(self, config):
        ...
        piece_num = 1
        self.input_channel = 2*piece_num+3
        self.action_size = self.height * self.width
        self.input_size = self.height * self.width * self.input_channel

        # neural network
        self.batch_size = config['batch_size']
        self.epochs = config['epochs'] # *8 for no symmetry
        self.nnet = NeuralNetWorkWrapper(config['lr'], config['l2'], config['num_layers'],
                                         config['num_channels'], self.height, self.width, 
            ...
            with open(file_path, 'rb') as binfile:
                ...
                for st in range(step):
                    for i in range(self.input_size):
                        ...
                        data=struct.unpack('f', data)[0]
                        board_new[st][i] = data
                board_new = board_new.reshape((-1,self.input_channel,self.height,self.width))
