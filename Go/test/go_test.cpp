...
int main() {
    ...
    Go game(width, height, Go::FirstColor);
    ...
    while (true) {
        ...
        if (status.first == 1) {
            std::string winner = (status.second == Go::FirstColor) ? "黑方" : "白方";
            std::cout << "\n游戏结束! " << winner << "获胜!" << std::endl;
            break;
        }
        
        std::string color_str = (game.get_current_color() == Go::FirstColor) ? "黑方" : "白方";
        ...
        if (input == "pass") {
            ...
            if (pass_count >= 2) {
                ...
                std::string winner = (final_status.second == Go::FirstColor) ? "黑方" : "白方";
