...
{
    MCTS ma(a, NUM_MCT_THREADS, C_PUCT, a_mct_sims, C_VIRTUAL_LOSS, ChineseChess::action_size);
    MCTS mb(b, NUM_MCT_THREADS, C_PUCT, b_mct_sims, C_VIRTUAL_LOSS, ChineseChess::action_size);
    int step = 0;
    ...
    if ((game_state.second == ChineseChess::FirstColor && a_first) || (game_state.second == ChineseChess::SecondColor && !a_first))
    {
        cout << "winner = a" << endl;
        win_table[0]++;
    }
    else if ((game_state.second == ChineseChess::FirstColor && !a_first) || (game_state.second == ChineseChess::SecondColor && a_first))
    {
        cout << "winner = b" << endl;
        win_table[1]++;
    }
...
int main(int argc, char *argv[])
{
    if (strcmp(argv[1], "prepare") == 0)
    {
        ...
        random_mcts_logger_writer << 100 << " " << 100; // first:random mcts number, second:neural network mcts number
